...
def evaluate_parenthesis(tokens):
    while(1):
        ...
        tokens_inside_bracket = list_tokensInsideBracket[0] #()の中身
        bracket_left_index = list_tokensInsideBracket[1] #(の位置
        bracket_right_index = list_tokensInsideBracket[2] #)の位置
        if bracket_right_index == 0: # ()がないのでbreak
            break
        else:
...
def test(line, expectedAnswer):  #test
    ...
    actualAnswer = evaluate(tokens)  #()がなくなったら普通に計算
...
while True:
    ...
    tokens = evaluate_parenthesis(tokens) #最も内側の()の中身を計算 をかっこがなくなるまで繰り返す
    answer = evaluate(tokens)  #()がなくなったら普通に計算
    print "answer = %f\n" % answer
